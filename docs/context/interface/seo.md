...
| `/people-search` + `/grokbot` | The launch-campaign landings, bundled files served by their own routes (`FileResponse`, no-cache). Both are first-class pages: canonical, OG tags, listed in `_SITEMAP_PAGES` at 0.8 — `/people-search` is "Claude for people search" (the enrichment launch film's destination), `/grokbot` the "Grok Bot for Outreach" animatic. Their asset paths are **relative** (`media/…`, `logos/…`) so the same file previews from `file://`; that only holds while the routes stay slashless. Both are registered in `bootstrap.py`'s route-ownership manifest like every other route. |
