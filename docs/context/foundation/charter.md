...
| Not built | automatic routing or failover **between providers** of one capability. `treg catalog search` compares them; **choosing is the caller's**. What treg *may* do (when a deployment turns it on, [capacity](../ops/capacity.md)): if its **own** account for a provider is out, serve the **same endpoint** through a relay — same request, same response shape, the relay's real price, disclosed via `X-Treg-Served-Via`; a team can opt out |
