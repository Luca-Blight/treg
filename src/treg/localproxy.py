...
# ---- the agent hook: one file, rewritten on every start/stop ---------------------------------
# `BASH_ENV` names a file bash sources at the start of every NON-interactive shell. Point an agent
# harness at this one (once, in its config) and every command the agent runs picks up the current
# proxy by itself — no eval anyone has to remember. oneCLI does the same thing in their Claude plugin.
#
# The indirection is the point: the port and token change on every `serve start`, so putting the
# variables straight into a harness config would go stale the first time the proxy restarts. The
# config names a FILE; only the file's contents change.
_ENV_SCRIPT = "env.sh"


def env_script_path() -> Path:
    return proxy_dir() / _ENV_SCRIPT


def write_env_script(env: dict[str, str] | None) -> Path:
    """Write the sourced file: exports while a proxy is running, `unset`s when it is not.

    Stopping must rewrite it rather than delete it. A file that still exported a dead port would
    break every agent command until someone noticed, and deleting it would leave already-running
    shells exporting the old values with nothing to correct them."""
    path = env_script_path()
    path.parent.mkdir(parents=True, exist_ok=True)
    if env:
        body = "\n".join(f"export {k}={_sh_quote(v)}" for k, v in sorted(env.items()))
        head = "# treg local proxy — generated by `treg serve`. Sourced via BASH_ENV; do not edit.\n"
    else:
        body = "\n".join(f"unset {k}" for k in sorted(proxy_env(0, "", "")))
        head = "# treg local proxy — not running. Generated by `treg serve`; do not edit.\n"
    fd = os.open(path, os.O_WRONLY | os.O_CREAT | os.O_TRUNC, 0o600)  # holds the proxy token
    with os.fdopen(fd, "w") as fh:
        fh.write(head + body + "\n")
    os.chmod(path, 0o600)
    return path
